# condensing boiler return heatmap up to 24 August
...
bw_rl_ww = pd.read_csv(os.path.join(
    'data', 'withWeather', 'bw_rl_ww.csv'), parse_dates=['Timestamp'])

# get columns that we need
bw_rl_ww = bw_rl_ww[['Timestamp', 'Value', 'TT_10']]

# add a column for day of the week
bw_rl_ww['Day of week'] = bw_rl_ww['Timestamp'].dt.day_of_week

# add a column for hour of day
bw_rl_ww['Hour of day'] = bw_rl_ww['Timestamp'].dt.hour

# median system temperatures for each hour, each day of the week (e.g. Tuesday 18:00:00 [to 18:59:59])
medianSys = bw_rl_ww.groupby(['Day of week', 'Hour of day']).agg(
...
plt.title('Median BW Rücklauf temperatures by hour and day, April-August')
plt.show()