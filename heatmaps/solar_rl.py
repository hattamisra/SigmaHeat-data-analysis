# solar return flow up to 24 August
...
solar_rl = pd.read_csv(os.path.join(
    'data', 'withWeather', 'solar_vl.csv'), parse_dates=['Timestamp'])

# get columns that we need
solar_rl = solar_rl[['Timestamp', 'Value', 'TT_10']]

# add a column for day of the week
solar_rl['Day of week'] = solar_rl['Timestamp'].dt.day_of_week

# add a column for hour of day
solar_rl['Hour of day'] = solar_rl['Timestamp'].dt.hour

# median system temperatures for each hour, each day of the week (e.g. Tuesday 18:00:00 [to 18:59:59])
medianSys = solar_rl.groupby(['Day of week', 'Hour of day']).agg(
...
plt.title('Median Solar Rücklauf temperatures by hour and day, April-August')
plt.show()